...
{
	function alert_notice($alert = '', $type = '', $alt = FALSE) 
	{	
		/**
		 * An alias to my_config->alert() Obj
		 * @param  string $alert [the content of the alert to display]
		 * @param  string $type  [the type of alert to display]
		 * @param  string $alt   [my_config->alert() comes with an optional third parameter 
		 *                       If set, the alert is a smaller inline variant, the $alt can be set to TRUE or an id]
		 * @return string        Containing the formated html of the alert
		 */
		global $CI;

		return $CI->my_config->alert($alert, $type, $alt); 
	}
}
